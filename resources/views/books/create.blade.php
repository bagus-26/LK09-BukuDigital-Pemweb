...
<div class="max-w-3xl mx-auto">
    <div class="mb-8 flex items-center gap-5">
        ...
        <div>
            <h1 class="text-3xl font-extrabold text-zinc-900 tracking-tight">Tambah Arsip</h1>
            <p class="text-zinc-500 text-sm font-medium mt-1">Masukkan informasi literatur ke database</p>
        </div>
    </div>

    <div class="bg-white rounded-[2rem] shadow-soft border border-zinc-200/60 overflow-hidden">
        <form action="{{ route('books.store') }}" method="POST" enctype="multipart/form-data" class="p-8 sm:p-10">
            @csrf
            ...
            <div class="space-y-7">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="Contoh: Pemrograman Web Modern"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-7">
                    <div>
                        ...
                        <input type="number" name="year" value="{{ old('year') }}" min="1000" max="{{ date('Y') }}" placeholder="2024"
                               class="w-full px-5 py-3.5 bg-zinc-50/50 border border-zinc-200 rounded-2xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-zinc-900/5 focus:border-zinc-400 transition-all">
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" placeholder="Masukkan deskripsi singkat literatur..."
                              class="w-full px-5 py-4 bg-zinc-50/50 border border-zinc-200 rounded-2xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-zinc-900/5 focus:border-zinc-400 transition-all resize-y">{{ old('description') }}</textarea>
                </div>

                <div class="pt-3">
                    <label class="block text-sm font-extrabold text-zinc-800 mb-3">Cover Literatur <span class="text-zinc-400 font-medium ml-1">(Opsional)</span></label>
                    <div class="group relative flex flex-col items-center justify-center w-full px-6 py-10 border-2 border-zinc-200 border-dashed rounded-3xl hover:border-zinc-900 hover:bg-zinc-50 transition-all bg-white cursor-pointer" onclick="document.getElementById('cover').click()">
                        <div class="space-y-2 text-center flex flex-col items-center">
                            ...
                            <div class="text-sm text-zinc-600 font-bold">
                                Klik untuk mengunggah file cover
                            </div>
            ...
            <div class="mt-10 flex flex-col sm:flex-row gap-3 pt-6 border-t border-zinc-100">
                ...
                <button type="submit" class="flex-1 px-8 py-3.5 bg-zinc-900 hover:bg-zinc-800 text-white rounded-2xl text-sm font-extrabold transition-all shadow-md hover:shadow-lg text-center order-1 sm:order-2">Simpan ke Database</button>
            </div>
        </form>
    </div>
</div>
